...
export async function addStudent(state: AddStudentType, formData: FormData): Promise<AddStudentType> {
  try {
    ...
    console.error(err);
    if (err instanceof Error) return { ok: false, data: null, message: `Erro ao adicionar aluno: ${err.message}.` };
    return { ok: false, data: null, message: `Erro ao adicionar aluno.` };
  }
}
